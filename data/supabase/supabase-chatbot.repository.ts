...
/**
 * Invokes the customer-chatbot Edge Function with the signed-in user's JWT.
 * API keys stay server-side; this client only uses the public anon key + session.
 */
export class SupabaseChatbotRepository implements ChatbotRepository {
...
function parsePendingAction(raw: ChatPendingAction | null | undefined): ChatPendingAction | null {
  ...
  const type = raw.type;
  if (type !== 'join_queue' && type !== 'cancel_ticket') return null;
